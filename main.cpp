   ...
   // Load will fail if watchers did not valiate.
	...
	caseselector (classid)
	{
		...
		incaseof ("Domain:HTTPSVhost") :
			...
			{
				incaseof ("create") :
					if (! apache2Module::writehttpsvhost (data)) return 0;
					break;
				...
				incaseof ("delete") :
					if (! apache2Module::removevhost (data,true)) return 0;
					break;
...
void apache2Module::writewarningheader( file& f )
{
    f.writeln ("################################################################################");
...
// METHOD apache2Module::writevhost
// =========================================================================
bool apache2Module::writehttpsvhost(value &v)
{
	//
	// Write the config files for all vhosts given
	//
	...
	if (f.openwrite (fname))
	{
		...
		f.printf ("   DocumentRoot       %s/sites/%s%s/secure_html\n", 
				  homedir.cval(),
				  subdom.cval(),
				  v["Domain"]["id"].cval());
		...
		// this Virtual host
		//
		f.printf ("   <Directory %s/sites/%s%s/secure_html>\n",
				  homedir.cval(),
				  subdom.cval(),
				  v["Domain"]["id"].cval());
		...
		// Add include directives
		f.printf ("   Include %s/%s%s.https.inc/[^.#]*\n",
				  conf["config"]["htservice:vhosts_dir"].cval(),
				  subdom.cval(),
				  v["Domain"]["id"].cval());
		...
		//  TODO: this todo left unintentionally blank
		string ifile;
		ifile.printf ("%s%s.https.conf",  subdom.cval(),
					  v["Domain"]["id"].cval());
		...
		// Construct file to write
		string certname, certpath;
		certname.printf ("%s%s.pem", 
					  subdom.cval(),
					  v["Domain"]["id"].cval());
		...
		//
		string adirname;
		adirname.printf ("%s/%s%s.https.inc", 
						 conf["config"]["htservice:vhosts_dir"].cval(),
						 subdom.cval(),
					  	v["Domain"]["id"].cval());
		...
		string userdir = "sites/%s%s/secure_html"
								%format (subdom,v["Domain"]["id"]);
...
bool apache2Module::removevhost (value &v, bool https)
{
	...
	foreach (vhost, v[https?"Domain:HTTPSVhost":"Domain:Vhost"])
	{
		...
		// Construct file to write
		fname.printf ( https ? "%s/%s%s.https.conf" : "%s/%s%s.conf", 
				   	  conf["config"]["htservice:vhosts_dir"].cval(),
				   	  subdom.cval(),
				   	  v["Domain"]["id"].cval());
		...
		string adirname;
		adirname.printf (https ? "%s/%s%s.https.inc" : "%s/%s%s.inc", 
						 conf["config"]["htservice:vhosts_dir"].cval(),
						 subdom.cval(),
						 v["Domain"]["id"].cval());	
